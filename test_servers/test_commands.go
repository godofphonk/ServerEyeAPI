...
func main() {
	...
	fmt.Println(`    -d '{"type":"metrics_stats","payload":{}}' | jq`)
	fmt.Println()
	fmt.Println(`  curl -X POST http://localhost:8084/api/servers/management/command \`)
	fmt.Println(`    -H "Content-Type: application/json" \`)
	fmt.Println(`    -d '{"type":"refresh_aggregates","payload":{"granularity":"all"}}' | jq`)
	fmt.Println()
	fmt.Println(`  curl -X POST http://localhost:8084/api/servers/management/command \`)
	fmt.Println(`    -H "Content-Type: application/json" \`)
	fmt.Println(`    -d '{"type":"cleanup_old_metrics","payload":{"older_than":"90 days","dry_run":true}}' | jq`)
